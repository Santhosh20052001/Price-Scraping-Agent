...
# --- Streamlit Setup ---
st.set_page_config(page_title="Best Buy Price Agent", layout="wide")
st.title("📈 Real-Time Best Buy Price Agent (TXT Input)")

# --- Sidebar controls ---
input_txt = st.sidebar.text_input("Input TXT path", "bestbuy_uids.txt")
interval = st.sidebar.number_input("Scrape interval (seconds)", min_value=10, max_value=600, value=30)
run_agent = st.sidebar.toggle("Start AI Agent", value=False)
...
def display_cards():
    ...
    df_latest_per_product = df_combined.sort_values("ScrapedAt").groupby("DFN").tail(1)
    cols_per_row = 3

    for i, (_, row) in enumerate(df_latest_per_product.iterrows()):
        if i % cols_per_row == 0:
            cols = cards_placeholder.columns(cols_per_row)

        dfn = row["DFN"]
        current_price = row["SalePrice"]
        df_history = df_combined[df_combined["DFN"] == dfn].sort_values("ScrapedAt")
...
if run_agent:
    ...
    while True:
        try:
            # Scrape latest prices (from TXT)
            df_latest = scrape_prices_once(input_txt)
            ...
            df_latest_per_product = st.session_state.df_combined.sort_values("ScrapedAt").groupby("DFN").tail(1)
            df_latest_per_product = df_latest_per_product.iloc[:3]
            table_placeholder.dataframe(df_latest_per_product, use_container_width=True)
