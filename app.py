...
uploaded_file = st.sidebar.file_uploader(
    "Upload CSV with 'ProductCode' & 'DFN' columns", type="csv"
)
...
def scrape_prices_once(input_file):
    ...
    df_input = df_input.iloc[:3]  # optional: limit for testing

    if "ProductCode" not in df_input.columns or "DFN" not in df_input.columns:
        st.error("CSV must have 'ProductCode' and 'DFN' columns.")
        return pd.DataFrame()

    run_timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    sale_prices, original_prices = [], []

    for code in df_input["ProductCode"]:
        sale, original = get_bestbuy_prices(code)
        sale_prices.append(sale)
        original_prices.append(original)
...
if run_agent:
    if uploaded_file is None:
        st.warning("Upload a CSV to start the agent.")
    else:
        st.sidebar.success("Agent is running...")
        iteration = 1
        while True:
            try:
                df_latest = scrape_prices_once(uploaded_file)
                append_to_csv(df_latest, output_csv)
